    ...
    while (attributes.window.pollEvent(event) || game_event.type == GameEvent::EventType::wait_event) {
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
            if (event.mouseButton.button == sf::Mouse::Left) {
                ...
                sf::Vector2f abs_mouse_pos = sf::Vector2f(event.mouseButton.x * attributes.default_window_size.x / window_size.x,
                                                      event.mouseButton.y * attributes.default_window_size.y / window_size.y);
                if (!game_event.card)
                    game_event.card = pressed_card(abs_mouse_pos);

                if (!game_event.base)
                    game_event.base = pressed_base(abs_mouse_pos);
                if (!game_event.base)
                    game_event.deck = pressed_deck(abs_mouse_pos);
            }
        }
